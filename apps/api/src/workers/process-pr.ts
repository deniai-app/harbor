...
function buildDefaultReviewingCommentCore(params: {
  ...
  return [
    `対象: ${params.owner}/${params.repo}#${params.pullNumber}`,
    "",
    "<details>",
    "<summary>進行概要（作成中）</summary>",
    "",
    "### 現在の進捗",
    "- 変更ファイルと差分を収集中",
    "- リスクと境界条件を分析中",
...
function buildCompletedReviewingComment(params: {
  ...
  const statusLine = params.error ? "## Harbor 審査完了（異常あり）" : "## Harbor 審査完了";

  let resultSummary = "- 結果: 審査処理を完了しました。";
  if (params.error) {
    resultSummary = `- 結果: 失敗 (${String((params.error as Error)?.message ?? params.error)})`;
  } else if (params.outcome?.status === "posted") {
    resultSummary = `- 結果: レビューを投稿 (${params.outcome.inlineCommentCount} 件の行コメント)。`;
  } else if (params.outcome?.status === "no_suggestions") {
    resultSummary = "- 結果: 安全に投稿できる提案は見つかりませんでした。";
  } else if (params.outcome?.status === "approved") {
    resultSummary = "- 結果: 承認（対応が必要な指摘なし）。";
  } else if (params.outcome?.status === "skipped_no_llm") {
    resultSummary = "- 結果: LLM プロバイダー未設定のため処理をスキップ。";
  }
  ...
  return [
    ...
    "",
    "<details>",
    "<summary>審査実行結果</summary>",
    "",
    "### 実行結果",
    resultSummary,
    bodySummary,
    `- 処理時間: 約${elapsedSeconds}秒`,
    "",
    "</details>",
    "",
    "_この進行コメントは最終状態へ更新済みです。_",
    "",
    "<!-- deniai:reviewing:end -->",
  ].join("\n");
}
